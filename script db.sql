...
DELETE FROM LOCALIDADES WHERE ID = 8

select TOP 1 ID from DOMICILIOS ORDER BY ID DESC

ALTER TABLE PROVEEDORES_x_PRODUCTO NOCHECK CONSTRAINT FK__PROVEEDOR__IDPRO__7F2BE32F

Select L.ID from LOCALIDADES AS L
Inner join DOMICILIOS AS D ON D.IDLOCALIDAD = L.ID
where L.NOMBRE like 'San Fernando' and D.ID = 1

select * from EMPLEADOS

ALTER TABLE EMPLEADOS
ADD FECHNAC DATE NULL

Select P.*, D.CALLE, D.ALTURA, L.NOMBRE as LOCALIDAD, D.PISO, D.DEPARTAMENTO, L.CODPOSTAL, L.PARTIDO, L.ID AS IDLOCALIDAD, D.ENTRECALLE1, D.ENTRECALLE2 from PROVEEDORES AS P LEFT JOIN DOMICILIOS AS D ON D.ID = P.IDDOMICILIO LEFT JOIN LOCALIDADES AS L ON L.ID = D.IDLOCALIDAD
Select ID FROM DOMICILIOS where CALLE LIKE '%Santa Ritaaa%' AND ALTURA LIKE 2424 AND ENTRECALLE1 LIKE '' AND ENTRECALLE2 LIKE ''

ALTER TABLE PROVEEDORES_X_PRODUCTO NOCHECK CONSTRAINT FK__PROVEEDOR__IDPRO__7F2BE32F DELETE FROM DOMICILIOS WHERE ID = 16 ALTER TABLE PROVEEDORES_x_PRODUCTO CHECK CONSTRAINT FK__PROVEEDOR__IDPRO__7F2BE32F

Select C.*, D.CALLE, D.ALTURA, L.NOMBRE as LOCALIDAD, D.PISO, D.DEPARTAMENTO, L.CODPOSTAL, L.PARTIDO, L.ID AS IDLOCALIDAD, D.ENTRECALLE1, D.ENTRECALLE2 from CLIENTES AS C LEFT JOIN DOMICILIOS AS D ON D.ID = C.IDDOMICILIO LEFT JOIN LOCALIDADES AS L ON L.ID = D.IDLOCALIDAD

Select E.*, D.CALLE, D.ALTURA, L.NOMBRE as LOCALIDAD, D.PISO, D.DEPARTAMENTO, L.CODPOSTAL, L.PARTIDO, L.ID AS IDLOCALIDAD, D.ENTRECALLE1, D.ENTRECALLE2, U.USUARIO from EMPLEADOS AS E LEFT JOIN DOMICILIOS AS D ON D.ID = E.IDDOMICILIO LEFT JOIN LOCALIDADES AS L ON L.ID = D.IDLOCALIDAD LEFT JOIN USUARIOS AS U ON U.ID = E.IDUSUARIO

INSERT INTO USUARIOS (USUARIO, CONTRASEÑA) VALUES ('TATO', '1234') 
select SCOPE_IDENTITY();


SELECT * FROM USUARIOS

INSERT INTO USUARIOS (USUARIO, CONTRASEÑA) VALUES ('Tato', '1234') SELECT SCOPE_IDENTITY();
